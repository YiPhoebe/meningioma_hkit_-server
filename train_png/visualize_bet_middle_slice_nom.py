...
t1c_dir = "/home/iujeong/brain_meningioma/bet_output/bet_output_train"
gtv_dir = "/home/iujeong/brain_meningioma/bet_output/bet_output_train"

# 출력 PNG 저장 경로
output_dir = "/home/iujeong/brain_meningioma/train_png/bet_axial_png"
...
for f in sorted(os.listdir(t1c_dir)):
    ...
    case_id = f.replace("_t1c_bet.nii.gz", "")
    gtv_name = case_id + "_t1c_gtv.nii.gz"
    gtv_path = os.path.join(gtv_dir, gtv_name)

    if not os.path.exists(gtv_path):
        print(f"[!] GTV file not found: {gtv_path}")
        continue
