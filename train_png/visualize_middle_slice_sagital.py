...
# 출력 PNG 저장 경로
output_dir = "/home/iujeong/brain_meningioma/train_png/segital_png"
...
for f in sorted(os.listdir(t1c_dir)):
    ...
    try:
        ...
        plt.subplot(1, 2, 1)
        plt.imshow(t1c_slice, cmap="gray", origin="lower")
        plt.title(f"{case_id} T1c (Sagittal)")
        plt.axis("off")

        plt.subplot(1, 2, 2)
        plt.imshow(t1c_slice, cmap="gray", origin="lower")
        plt.imshow(gtv_slice, cmap="Reds", alpha=0.4, origin="lower")
        plt.title(f"{case_id} + GTV (Sagittal)")
        plt.axis("off")
