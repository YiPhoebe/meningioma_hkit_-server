...
for pid, bet_dir in all_cases:
    ...
    # shape mismatch 처리: BET 마스크를 GTV 마스크 크기에 맞게 pad 또는 crop
    if gtv_mask.shape != bet_mask.shape:
        diff = np.array(gtv_mask.shape) - np.array(bet_mask.shape)
        pad_width = [(0, d) if d > 0 else (0, 0) for d in diff]
        bet_mask = np.pad(bet_mask, pad_width, mode='constant', constant_values=0)
        # bet_mask가 더 클 경우 crop
        bet_mask = bet_mask[:gtv_mask.shape[0], :gtv_mask.shape[1], :gtv_mask.shape[2]]
